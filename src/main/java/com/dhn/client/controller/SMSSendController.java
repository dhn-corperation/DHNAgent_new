...
@Component
@Slf4j
public class SMSSendController implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void SendProcess(){
        if (isStart && !isProc) {
            ...
            String send_group = "S"+now.format(formatter);
...
//                                log.info("KAO group_no {} 상태 업데이트 완료: {}", group_no, status);
                                ...
                                if(responseMap.get("code").equals("00")){
                                    log.info("SMS 메세지 전송 완료( {} ) : {} 건 ( {} )",send_group, dataList.size(),responseMap.get("code"));
                                    requestService.msgSendSuccess(param);
                                }else{
                                    log.error("SMS 메세지 전송 오류(Data ERR) : " + responseMap.get("code") + " / " + responseMap.get("message"));
                                    param.setMessage(responseMap.get("message").toString());
                                    requestService.msgSendFail(param);
                                }

                            } catch (Exception e) {
                                log.error("SMS 메세지 전송 오류(Response) : " + e.getMessage());
